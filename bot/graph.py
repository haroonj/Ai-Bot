...
def create_graph() -> StateGraph:
    ...
    workflow.add_conditional_edges(
        ...
        lambda state: "handle_multi_turn_return" if state.get("intent") == "initiate_return" and not state.get("tool_error") and state.get("next_node") != "generate_response" # Check explicit next node later
    ...
    workflow.add_conditional_edges(
        "handle_multi_turn_return",
         # Decide based on whether clarification is needed or ready for next step
        lambda state: "generate_response" if state.get("needs_clarification")
                      elif state.get("intent") == "return_reason_provided": "execute_tool" # Submit the return via execute_tool
                      else "generate_response", # Should not happen often, fallback
        {
...
try:
    ...
    try:
        ...
        logger.warning(f"Could not draw graph diagram (requires graphviz and potentially other libs): {draw_error}")
        try:
             # Fallback to ASCII

...
MOCK_API_URL = str(settings.mock_api_base_url) # Ensure it's a string

# --- HTTP Client ---
# Using httpx for async/sync requests. Sync for simplicity here.
client = httpx.Client(base_url=MOCK_API_URL, timeout=10.0)

# --- Tool Input Schemas (Good Practice) ---
...
def get_order_details(order_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Only return items if the order is delivered and eligible for return
        if details.get("delivered"):
            return details
        else:
            return {"error": f"Order {order_id} is not yet delivered or not eligible for return based on details.", "details": details}
    except httpx.HTTPStatusError as e:
...
def initiate_return_request(order_id: str, sku: str, reason: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return response.json() # Should contain return_id, status, message
    except httpx.HTTPStatusError as e:
        logger.warning(f"API Error initiating return for {order_id}, sku {sku}: {e.response.status_code} - {e.response.text}")
        # Return the error message from the API response body if possible
        try:
...
def knowledge_base_lookup(query: str) -> str:
    ...
    try:
        ...
        # Limit context size if necessary, though often handled by LLM context window
...
# --- List of all tools ---
# This list will be used by the LLM for routing/tool selection
available_tools = [
    get_order_status,
    get_tracking_info,
    get_order_details, # Needed internally for returns, might not be directly exposed
    initiate_return_request,
    knowledge_base_lookup,
]

# Bind tools to the LLM for easier tool calling
# Make sure the LLM supports the tool calling method you intend to use
# (e.g., OpenAI Functions, Anthropic Tools, Gemini Tools)
from .llm import llm

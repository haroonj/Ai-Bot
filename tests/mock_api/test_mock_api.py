...
#     response = client.get("/orders/123/status")
#     assert response.status_code == 200
#     assert response.json() == {"order_id": "123", "status": "Shipped"}
#
#
...
#     response = client.get("/orders/123/tracking")
#     assert response.status_code == 200
...
#     response = client.get("/orders/456/tracking")
#     assert response.status_code == 200
#     assert response.json() == {"order_id": "456", "tracking_number": None, "carrier": None,
...
#     response = client.get("/orders/789/details")
#     assert response.status_code == 200
#     data = response.json()
...
#     payload = {"order_id": "789", "sku": "ITEM004", "reason": "Test reason"}
...
#     payload = {"order_id": "789", "sku": "WRONGITEM"}
#     response = client.post("/returns", json=payload)
#     assert response.status_code == 404
#     assert response.json() == {"detail": "Item SKU WRONGITEM not found in order 789"}
#
#
# def test_initiate_return_not_delivered(client):
#     payload = {"order_id": "123", "sku": "ITEM001"}
#     response = client.post("/returns", json=payload)
